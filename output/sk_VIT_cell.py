...
def combinar_arrays(arrays):
	...
	combinaciones = list(combinations(arrays, 3))
	
	arrays_combinados = []
	
	for combo in combinaciones:
		array_1, array_2, array_3 = combo
		
		# Concatenar los dos arrays en uno solo
		array_combinado = np.concatenate((array_1, array_2, array_3))
...
predictions_0 = {}
#__CLOUDBOOK:NONSHARED__
vit_classifier = [None, None, None, None]
to_predict_models = []
#__CLOUDBOOK:PARALLEL__
def skynnet_train_0(sk_i):
	global vit_classifier
	...
	num_heads = 4
	transformer_units = [128, embeddings_dim]
	...
	def create_vit_classifier(input_shape, num_classes: int, image_size: int, patch_size: int, num_patches: int, projection_dim: int, dropout: float, n_transformer_layers: int, num_heads: int, transformer_units: List[int], mlp_head_units: List[int], normalization: bool=False):
		nonlocal _DATA_TRAIN_X, _DATA_TRAIN_Y
		grupos_de_categorias = dividir_array_categorias(_DATA_TRAIN_Y, 4, 4)
		combinacion_arrays = combinar_arrays(grupos_de_categorias)[sk_i]
		print(f"grupos de categorias {grupos_de_categorias} y combinacion_arrays {combinacion_arrays}")
		...
		vit_classifier[sk_i] = keras.Model(inputs=inputs, outputs=logits)
		return vit_classifier[sk_i]'''
	#print('ABM: n_classes=', n_classes)
	vit_classifier[sk_i] = create_vit_classifier(input_shape=input_shape, num_classes=n_classes, image_size=image_size, patch_size=patch_size, num_patches=num_patches, projection_dim=embeddings_dim, dropout=0.2, n_transformer_layers=transformer_layers, num_heads=num_heads, transformer_units=transformer_units, mlp_head_units=mlp_head_units)
	'''optimizer = tf.optimizers.Adam(learning_rate=lr)
	vit_classifier[sk_i].compile(optimizer=optimizer, loss=keras.losses.SparseCategoricalCrossentropy(from_logits=False), metrics=[keras.metrics.SparseCategoricalAccuracy(name='accuracy')])
	print(f'Parametros del modelo: {vit_classifier[sk_i].count_params()}')'''
	'''start = time.time()
	history = vit_classifier[sk_i].fit(_DATA_TRAIN_X, _DATA_TRAIN_Y, epochs=num_epocas, batch_size=batch_size, shuffle=True, validation_split=my_split)
	end = time.time()
...
def skynnet_prediction_global_0():
	...
	if _DATA_TEST_Y[0].shape != ():
		...
		cce_orig = cce(_DATA_TEST_Y, prediction).numpy()
		print('============================================')
		...
		print('============================================')
	for i in [0, 1, 2, 3]:
		print('class ', i, ', num_elements=', len(np.where(_DATA_TEST_Y == i)[0]), '  (', len(np.where(_DATA_TEST_Y == i)[0]) / len(_DATA_TEST_Y) * 100.0, '%)')
