...
#__CLOUDBOOK:LOCAL__
def adaptImages(x_train,a,samples,data_type,h2,w2,channels2):
    j=0
...
def load_data():
    ...
    global testy
    print("Loading data")
    ds_train, ds_test = tfds.load('caltech101', 
                              data_dir='caltech101',
    ...
    x_train = np.array([x[0] for x in ds_train_npy])
    y_train = np.array([x[1] for x in ds_train_npy])
    ...
    valid_samples=adaptImages(x_train,a,samples,data_type,h2,w2,channels2)

    a = a/255.0
    x_train = a

    ds_test_npy=tfds.as_numpy(ds_train)
    #testx = np.array(list(map(lambda x: x[0], ds_test_npy)))
    #testy = np.array(list(map(lambda x: x[1], ds_test_npy)))
    testx = np.array([x[0] for x in ds_test_npy])
    testy = np.array([x[1] for x in ds_test_npy])


    w2=227
    h2=227
    channels=testx[0].shape[2] # todos los elementos tienen mismo num canales
    samples=testx.shape[0]
    data_type= testx[0].dtype
    channels2=channels # es por si queremos pasar a bn, basta con poner channels2=1
    testx = np.zeros(h2*w2*channels2*samples, dtype=np.float32) #data_type)
    testx = testx.reshape((samples,h2,w2,channels2))
    valid_samples=adaptImages(testx,testx,samples,data_type,h2,w2,channels2)
